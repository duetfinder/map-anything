...
- 仅在有效像素上估计一个全局偏移 `b`
- 用 `pred_z_aligned = pred_z + b` 后再计算高度误差
...
| 指标名 | 任务阶段 | 定义对象 | 计算空间 / 对齐方式 | 是否 metric | 指标含义 | 当前建议角色 |
|---|---|---|---|---|---|---|
| `rs_height_mae` | RS-only | 卫星影像逐像素高度 | 在有效像素上先估计每个 scene 的 z-offset，并用 `pred_z_aligned = pred_z + b` 做对齐，然后计算 MAE | 是 | 衡量模型在卫星影像输入下恢复高度结构的平均绝对误差；因为允许 z 轴整体偏移，所以更符合当前 RS 任务定义 | 主指标 |
...
| 指标名 | 是否需要 `view0` 参考系变换 | 是否需要联合尺度归一化 | 是否需要 z-offset 对齐 | 是否需要恢复到全局坐标系 | 是否使用后验 `SE(3)` / `Sim(3)` 最优对齐 |
|---|---|---|---|---|---|
...
- 第 `i` 个视图的 GT 世界点为 `P_i^gt`
- 第 0 个视图的预测位姿为 `T_0^pred`
- 第 `i` 个视图的预测世界点为 `P_i^pred`

则：

```text
	ilde{P}_i^gt = (T_0^gt)^{-1} P_i^gt
	ilde{P}_i^pred = (T_0^pred)^{-1} P_i^pred
```
...
设一个 scene 内所有有效点组成集合 `M`，点为 `P_k`，则归一化因子为：

```text
s = (1 / |M|) * Σ_{k in M} ||P_k||_2
```
...
```text
e_i = ||\hat{P}_i^pred - \hat{P}_i^gt||_2 / (||\hat{P}_i^gt||_2 + \epsilon)
```

然后对有效像素平均：

```text
pointmaps_abs_rel = (1 / |M|) * Σ_{i in M} e_i
```

其中：

- `\hat{P}_i^gt` 和 `\hat{P}_i^pred` 表示经过 `view0` 参考系变换和联合尺度归一化后的点
- `M` 为有效像素集合
...
```text
e_i = |\hat{z}_i^pred - \hat{z}_i^gt| / (|\hat{z}_i^gt| + \epsilon)
```

整体指标为：

```text
z_depth_abs_rel = (1 / |M|) * Σ_{i in M} e_i
```
...
```text
pose_ate_rmse = sqrt( (1/N) * Σ_k ||t_k^pred - t_k^gt||_2^2 )
```
...
```text
pose_auc_5 = \int_0^{5^\circ} InlierRate(\tau) \, d\tau
```
...
```text
\theta_i = arccos( clamp( <r_i^pred, r_i^gt>, -1, 1 ) )
```

整体指标：

```text
ray_dirs_err_deg = (1 / |M|) * Σ_{i in M} \theta_i
```
...
```text
metric_scale_abs_rel = |s^pred - s^gt| / (|s^gt| + \epsilon)
```
...
- `P_i^gt` 为 GT 全局坐标下的 3D 点
- `P_i^pred` 为 prediction 在真实尺度、真实坐标语义下的 3D 点

则：

```text
metric_point_l1 = (1 / |M|) * Σ_{i in M} ||P_i^pred - P_i^gt||_1
```
...
```text
b = (1 / |M|) * Σ_{i in M} (h_i^gt - h_i^pred)
```

然后得到对齐后的预测高度：

```text
h_i^{pred, aligned} = h_i^pred + b
```
...
```text
rs_height_mae = (1 / |M|) * Σ_{i in M} |h_i^{pred, aligned} - h_i^gt|
```

### 18.13 `rs_height_rmse`

```text
rs_height_rmse = sqrt( (1 / |M|) * Σ_{i in M} (h_i^{pred, aligned} - h_i^gt)^2 )
```
...
- Aerial 在 `view0` 参考系中的预测点为 `P_i^{aerial,view0}`
- 预测的 `view0 -> global` 变换为 `T_{view0 \rightarrow global}^{pred}`

则：

```text
P_i^{aerial,global} = T_{view0 \rightarrow global}^{pred} P_i^{aerial,view0}
```
...
(1 / (|M_a| + |M_r|)) *
[
  Σ_{i in M_a} ||P_i^{aerial,global,pred} - P_i^{aerial,global,gt}||_1
  +
  Σ_{j in M_r} ||P_j^{rs,global,pred} - P_j^{rs,global,gt}||_1
]
```
...
### 18.16 为什么当前不建议对主指标做后验 `SE(3)` / `Sim(3)` 最优对齐
...
- 如果后续需要，可以额外报告一组分析指标，例如：
  - `Sim3-aligned metric_point_l1`
  - `Sim3-aligned joint_global_point_l1`
