...
SPLIT_PROVIDER_CSV=${SPLIT_PROVIDER_CSV:-}
...
OUTPUT_DIR=${OUTPUT_DIR:-'${root_experiments_dir}/mapanything/training/vigor_chicago/p3_pi3_joint_input_500_2gpu_all_zero_covis'}
...
REMOTE_PROVIDER_MAP_ARGS=()
if [ -n "${SPLIT_PROVIDER_CSV}" ]; then
    REMOTE_PROVIDER_MAP_ARGS+=(
        "dataset.vigor_chicago_joint_rs_aerial.train.remote_provider_map_csv=${SPLIT_PROVIDER_CSV}"
        "dataset.vigor_chicago_joint_rs_aerial.val.remote_provider_map_csv=${SPLIT_PROVIDER_CSV}"
        "dataset.vigor_chicago_joint_rs_aerial.test.remote_provider_map_csv=${SPLIT_PROVIDER_CSV}"
    )
fi
...
PYTHONPATH=. CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node "${NUM_GPUS}" \
    ...
    dataset.vigor_chicago_joint_rs_aerial.train.view_sampling_mode=zero_covis \
    dataset.vigor_chicago_joint_rs_aerial.val.view_sampling_mode=zero_covis \
    dataset.vigor_chicago_joint_rs_aerial.test.view_sampling_mode=zero_covis \
    dataset.vigor_chicago_joint_rs_aerial.train.covisibility_thres=0.0 \
    dataset.vigor_chicago_joint_rs_aerial.val.covisibility_thres=0.0 \
    dataset.vigor_chicago_joint_rs_aerial.test.covisibility_thres=0.0 \
