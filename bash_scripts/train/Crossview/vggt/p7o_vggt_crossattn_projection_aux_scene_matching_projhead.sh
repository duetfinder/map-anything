...
MAX_REMOTE_TOKENS=${MAX_REMOTE_TOKENS:-128}
...
REMOTE_CONTROL_RANKING_LOSS_WEIGHT=${REMOTE_CONTROL_RANKING_LOSS_WEIGHT:-0.0}
REMOTE_CONTROL_RANKING_MARGIN=${REMOTE_CONTROL_RANKING_MARGIN:-0.0}
REMOTE_CONTROL_RANKING_MODES=${REMOTE_CONTROL_RANKING_MODES:-[shuffled]}
REMOTE_CONTROL_BLANK_VALUE=${REMOTE_CONTROL_BLANK_VALUE:-0.5}
REMOTE_CONTROL_RANKING_SAME_LOSS=${REMOTE_CONTROL_RANKING_SAME_LOSS:-aerial_grad}
REMOTE_BLANK_PRESERVE_LOSS_WEIGHT=${REMOTE_BLANK_PRESERVE_LOSS_WEIGHT:-0.0}
REMOTE_SCENE_MATCHING_LOSS_WEIGHT=${REMOTE_SCENE_MATCHING_LOSS_WEIGHT:-0.5}
REMOTE_SCENE_MATCHING_TEMPERATURE=${REMOTE_SCENE_MATCHING_TEMPERATURE:-0.07}
REMOTE_SCENE_MATCHING_GATHER_DISTRIBUTED=${REMOTE_SCENE_MATCHING_GATHER_DISTRIBUTED:-true}
REMOTE_SCENE_MATCHING_PROJECTION_DIM=${REMOTE_SCENE_MATCHING_PROJECTION_DIM:-128}
REMOTE_SCENE_MATCHING_PROJECTION_HIDDEN_SCALE=${REMOTE_SCENE_MATCHING_PROJECTION_HIDDEN_SCALE:-0.25}
...
RESUME=${RESUME:-false}
TRAIN_PARAMS=${TRAIN_PARAMS:-vggt_p7o_projection_aux_scene_matching_projhead}
LOSS_CONFIG=${LOSS_CONFIG:-vggt_loss_rs_joint_p7h_projection_aux_robust_branch}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-p7o_vggt_crossattn_projection_aux_scene_matching_projhead}
...
echo "P7O cross-attention projection_aux scene-matching projection-head"
...
echo "REMOTE_CONTROL_RANKING=${REMOTE_CONTROL_RANKING_LOSS_WEIGHT} MODES=${REMOTE_CONTROL_RANKING_MODES} MARGIN=${REMOTE_CONTROL_RANKING_MARGIN} SAME_LOSS=${REMOTE_CONTROL_RANKING_SAME_LOSS}"
echo "REMOTE_SCENE_MATCHING=${REMOTE_SCENE_MATCHING_LOSS_WEIGHT} TEMP=${REMOTE_SCENE_MATCHING_TEMPERATURE} GATHER=${REMOTE_SCENE_MATCHING_GATHER_DISTRIBUTED} PROJ_DIM=${REMOTE_SCENE_MATCHING_PROJECTION_DIM}"
...
PYTHONPATH=. CUDA_VISIBLE_DEVICES="${CUDA_DEVICES}" torchrun --master_port "${MASTER_PORT}" --nproc_per_node "${NUM_GPUS}" \
    ...
    +model.model_config.use_remote_scene_matching_projection_head=true \
    +model.model_config.remote_scene_matching_projection_dim=${REMOTE_SCENE_MATCHING_PROJECTION_DIM} \
    +model.model_config.remote_scene_matching_projection_hidden_scale=${REMOTE_SCENE_MATCHING_PROJECTION_HIDDEN_SCALE} \
    ...
    train_params.remote_control_ranking_loss_weight=${REMOTE_CONTROL_RANKING_LOSS_WEIGHT} \
    train_params.remote_control_ranking_margin=${REMOTE_CONTROL_RANKING_MARGIN} \
    train_params.remote_control_ranking_modes=${REMOTE_CONTROL_RANKING_MODES} \
    train_params.remote_control_blank_value=${REMOTE_CONTROL_BLANK_VALUE} \
    +train_params.remote_control_ranking_same_loss=${REMOTE_CONTROL_RANKING_SAME_LOSS} \
    train_params.remote_blank_preserve_loss_weight=${REMOTE_BLANK_PRESERVE_LOSS_WEIGHT} \
    train_params.remote_scene_matching_loss_weight=${REMOTE_SCENE_MATCHING_LOSS_WEIGHT} \
    train_params.remote_scene_matching_temperature=${REMOTE_SCENE_MATCHING_TEMPERATURE} \
    train_params.remote_scene_matching_gather_distributed=${REMOTE_SCENE_MATCHING_GATHER_DISTRIBUTED} \
