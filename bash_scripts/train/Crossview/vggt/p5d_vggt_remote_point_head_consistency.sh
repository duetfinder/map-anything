...
LAMBDA_REMOTE_SCALE_ALIGNED_PM=${LAMBDA_REMOTE_SCALE_ALIGNED_PM:-0.0}
REMOTE_SCALE_ALIGNED_PM_MAX_SCALE=${REMOTE_SCALE_ALIGNED_PM_MAX_SCALE:-10.0}
LAMBDA_REMOTE_SHAPE_NORM_PM=${LAMBDA_REMOTE_SHAPE_NORM_PM:-0.0}
LAMBDA_REMOTE_AXIS_NORM_PM=${LAMBDA_REMOTE_AXIS_NORM_PM:-0.0}
REMOTE_AXIS_NORM_Z_WEIGHT=${REMOTE_AXIS_NORM_Z_WEIGHT:-4.0}
LAMBDA_REMOTE_H=${LAMBDA_REMOTE_H:-0.0}
REMOTE_POINTMAP_TOP_N_PERCENT=${REMOTE_POINTMAP_TOP_N_PERCENT:-0.0}
AERIAL_LOSS_WEIGHT=${AERIAL_LOSS_WEIGHT:-1.0}
REMOTE_LOSS_WEIGHT=${REMOTE_LOSS_WEIGHT:-1.0}
...
PYTHONPATH=. CUDA_VISIBLE_DEVICES="${CUDA_DEVICES}" torchrun --nproc_per_node "${NUM_GPUS}" \
    ...
    loss.remote_scale_aligned_pointmap_loss_weight=${LAMBDA_REMOTE_SCALE_ALIGNED_PM} \
    loss.remote_scale_aligned_pointmap_max_scale=${REMOTE_SCALE_ALIGNED_PM_MAX_SCALE} \
    loss.remote_shape_normalized_pointmap_loss_weight=${LAMBDA_REMOTE_SHAPE_NORM_PM} \
    loss.remote_axis_normalized_pointmap_loss_weight=${LAMBDA_REMOTE_AXIS_NORM_PM} \
    loss.remote_axis_normalized_pointmap_z_weight=${REMOTE_AXIS_NORM_Z_WEIGHT} \
    loss.remote_height_loss_weight=${LAMBDA_REMOTE_H} \
    loss.remote_pointmap_top_n_percent=${REMOTE_POINTMAP_TOP_N_PERCENT} \
    loss.aerial_loss_weight=${AERIAL_LOSS_WEIGHT} \
    loss.remote_loss_weight=${REMOTE_LOSS_WEIGHT} \
