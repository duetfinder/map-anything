...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-p5g_vggt_split_late_fusion_${FUSION_TYPE}}
...
MODEL_EXTRA_ARGS=(
    ...
    'config_overrides=["machine=aws","model=vggt","model.model_config.use_split_remote_aggregator=true","model.model_config.remote_to_aerial_late_fusion_type='"${FUSION_TYPE}"'","model.model_config.remote_to_aerial_late_fusion_hidden_scale=0.25","model.model_config.remote_to_aerial_late_fusion_gate_init=0.0","model.model_config.remote_to_aerial_cross_attention_heads='"${CROSS_ATTENTION_HEADS}"'","model.model_config.remote_to_aerial_max_remote_tokens='"${MAX_REMOTE_TOKENS}"'"]'
)
