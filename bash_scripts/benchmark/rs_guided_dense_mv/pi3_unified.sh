...
OUTPUT_DIR=${OUTPUT_DIR:-'${root_experiments_dir}/mapanything/benchmarking/rs_guided_dense_mv/newyork/pi3_unified'}
...
PYTHONPATH=. CUDA_VISIBLE_DEVICES=0 python3 \
    ...
    dataset.vigor_chicago_rs_aerial_benchmark.remote.crop_mode=${REMOTE_CROP_MODE}   \
    dataset.vigor_chicago_rs_aerial_benchmark.remote.num_augmented_crops_per_sample=${REMOTE_NUM_AUG_CROPS} \
    dataset.vigor_chicago_rs_aerial_benchmark.remote.crop_scale_range=[${REMOTE_CROP_SCALE_MIN},${REMOTE_CROP_SCALE_MAX}]  \
    dataset.vigor_chicago_rs_aerial_benchmark.remote.image_resize_mode=${REMOTE_IMAGE_RESIZE_MODE}  \
    dataset.vigor_chicago_rs_aerial_benchmark.remote.label_resize_mode=${REMOTE_LABEL_RESIZE_MODE}   \
