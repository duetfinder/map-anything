...
它的 raw benchmark 结果目前最好：`RS-only rs_point_l1=292.558`，`Joint rs_point_l1=292.122`。但当前筛选主指标已经改为尺度无关的 `rs_point_l1_scale_aligned`；已补跑的正尺度对齐实验显示，近期 scale-free 配置基本停在 `95.49` 左右，没有形成明确突破。这并不代表 remote 点云已经足够好，只代表在 P7-P5B 这条线里，robust-overlap 仍是最值得保留的参考点。
